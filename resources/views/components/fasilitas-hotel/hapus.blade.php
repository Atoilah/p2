        ...
        <div class="modal-dialog modal-sm modal-dialog-top" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Hapus Kamar  {{ $data->namaFasilitas }}</h5>
                    <a href="#" class="close" data-bs-dismiss="modal" aria-label="Close">
                        <em class="icon ni ni-cross"></em>
                    </a>
                </div>
                <div class="modal-body">
                    <form method="POST" action="{{route('fasilitas-hotel.destroy',$data->id)}}" class="form-validate is-alter" enctype="multipart/form-data">
                        @method('DELETE')
                        @csrf
